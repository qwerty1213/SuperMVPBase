...
public class GridImageAdapter extends
    ...
    public GridImageAdapter(Activity mActivity, onAddPicClickListener mOnAddPicClickListener) {
...
//            txtNum = view.findViewById(R.id.txt_num);
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int position) {
        ...
        if (getItemViewType(position) == TYPE_CAMERA) {
            viewHolder.mImg.setImageResource(R.mipmap.addimg_3x);
...
//            viewHolder.txtNum.setText(position + " / " + 4);
        } else {
...
//                        notifyItemRangeChanged(position, list.size());
            ...
            if (media.isCut() && !media.isCompressed()) {
                path = media.getCutPath();//裁剪过的图片
                L.i("裁剪过的图片————>" + media.getCutPath());
            } else if (media.isCompressed() || (media.isCut() && media.isCompressed())) {
                path = media.getCompressPath();                // 压缩过,或者裁剪同时压缩过,以最终压缩过图片为准
                L.i("压缩过,或者裁剪同时压缩过,以最终压缩过图片为准————>" + media.getCompressPath());
            } else {
            ...
            if (media.isCompressed()) {
                L.i("compress image result————>" + new File(media.getCompressPath()).length() / 1024 + "k");
                L.i("压缩地址————>" + media.getCompressPath());
            }
            int pictureType = PictureMimeType.isPictureType(media.getPictureType());
            if (media.isCut()) {
                L.i("裁剪地址————>" + media.getCutPath());
            }
